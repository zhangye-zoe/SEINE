...
from tiseg.utils import (pre_eval_all_semantic_metric, pre_eval_to_sem_metrics, pre_eval_bin_aji, pre_eval_bin_pq,
                         ...
                         pre_eval_to_bin_aji, pre_eval_to_bin_pq, pre_eval_bin_hausd_dis, pre_eval_to_imw_hausd, pre_eval_to_hausd)
...
def draw_all(save_folder,
    ...
    plt.subplot(247)
    plt.imshow(colorize_seg_map(tc_sem_pred))
    plt.axis('off')
...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data_infos = self.load_annotations(self.img_dir, self.ann_dir, self.img_suffix, self.sem_suffix,
                                                self.inst_suffix, self.edge_suffix, self.split)
    ...
    def load_annotations(self, img_dir, ann_dir, img_suffix, sem_suffix, inst_suffix, edge_suffix, split=None):
        ...
        if split is not None:
            ...
            for img_name in mmcv.scandir(img_dir, img_suffix, recursive=True):
                ...
                edge_name = img_name.replace(img_suffix, edge_suffix)
                img_file_name = osp.join(img_dir, img_name)
                sem_file_name = osp.join(ann_dir, sem_name)
                inst_file_name = osp.join(ann_dir, inst_name)
                edge_file_name = osp.join(ann_dir, edge_name)
    ...
    def pre_eval(self, preds, indices, show=False, show_folder=None):
        ...
        for pred, index in zip(preds, indices):
            ...
            inst_gt = mmcv.imread(sem_file_name, flag='unchanged', backend='pillow')
            ...
            # np.save(f"/data4/zhangye/Tissue-Image-Segmentation-master/z_visual2/{img_name}_sem.npy", sem_pred)
            # np.save(f"/data4/zhangye/Tissue-Image-Segmentation-master/z_visual2/{img_name}_inst.npy", inst_pred)
            ...
            # np.save(f"/data4/zhangye/Tissue-Image-Segmentation-master/z_linghan_128/{img_name}.npy", inst_pred)
            # cv2.imwrite(f"/data4/zhangye/Tissue-Image-Segmentation-master/z_zhangye/{img_name}.png", colorize_seg_map(inst_pred))
            ...
            bin_hausd_eval_res = pre_eval_bin_hausd_dis(inst_pred, inst_gt)
            ...
            single_loop_results = dict(
                ...
                bin_hausd_pre_eval_res=bin_hausd_eval_res)


            pre_eval_results.append(single_loop_results)
            # print("pre eval results", pre_eval_results)
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        # instance metrics (hausd style)
        bin_hausd_pre_eval_results = ret_metrics.pop('bin_hausd_pre_eval_res')
        ret_metrics.update(pre_eval_to_hausd(bin_hausd_pre_eval_results))
        # for k, v in pre_eval_to_bin_aji(bin_aji_pre_eval_results).items():
        #     ret_metrics['b' + k] = v
        img_ret_metrics.update(pre_eval_to_imw_hausd(bin_hausd_pre_eval_results))
        ...
        vital_keys = ['Dice', 'Precision', 'Recall', 'Aji', 'DQ', 'SQ', 'PQ', 'InstDice', 'Hausd']
        mean_metrics = {}
        overall_metrics = {}
