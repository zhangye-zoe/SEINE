...
def pre_eval_bin_hausd_dis(inst_pred, inst_gt):
    ...
    pairwise_hausd = np.zeros([len(gt_id_list) - 1, len(pred_id_list) - 1], dtype=np.float64)
    ...
    for gt_id in gt_id_list:  # 0-th is background
        ...
        for pred_id in pred_target_overlap_id:
            ...
            inst_hausd_dis = hausdorff_distance(g_mask_union_crop, p_mask_union_crop, distance='chebyshev')
            pairwise_hausd[gt_id-1, pred_id-1] = inst_hausd_dis
    ...
    # if pairwise_hausd.shape[0] * pairwise_hausd.shape[1] == 0:
    #     return 0., 0.
    # paired_pred = np.argmin(pairwise_hausd, axis=1)
    # pairwise_iou = np.min(pairwise_hausd, axis=1)
    # # exlude those dont have intersection
    # paired_gt = np.nonzero(pairwise_hausd > 0.0)[0]
    # paired_pred = paired_pred[paired_gt]
    overall_hausd = (pairwise_hausd[paired_gt, paired_pred]).sum()

    return overall_hausd


def pre_eval_to_imw_hausd(pre_eval_results, nan_to_num=None):
    """Convert aji pre-eval overall intersection & pre-eval overall union to hausd score."""
    
    ret_metrics = {}
    # ret_metrics['Hausd'] = []
    inst_hausd = [np.sum(x) for x in pre_eval_results]
    # ret_metrics['Hausd'].append(inst_hausd)

    ret_metrics['Hausd'] = np.array(inst_hausd)

    if nan_to_num is not None:
        ret_metrics = OrderedDict(
            {metric: np.nan_to_num(metric_value, nan=nan_to_num)
             for metric, metric_value in ret_metrics.items()})
    # print("Hausd", ret_metrics)
    return ret_metrics

def pre_eval_to_hausd(pre_eval_results, nan_to_num=None):
    ...
    hausd = sum(pre_eval_results)

    ret_metrics = {'Hausd': hausd}
...
def pre_eval_to_imw_inst_dice(pre_eval_results, nan_to_num=None):
    ...
    for tp, fp, fn in zip(tp_list, fp_list, fn_list):
        inst_dice = 2 * tp / (2 * tp + fp + fn + 1e-6)
